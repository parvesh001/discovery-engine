...
describe('understandQuery', () => {
  ...
  it('returns parsed intent and usage on a valid first response', async () => {
  ...
  it('accepts a response with a populated location filter, captured exactly as named', async () => {
    ...
    expect(result.intent.filters.location).toBe('Manali');
    expect(callClaudeMock).toHaveBeenCalledTimes(1);
  });
  ...
  it('strips markdown code fences before parsing', async () => {
    ...
    expect(result.intent.semantic_query).toBe('somewhere cozy and quiet for a weekend');
  });
  ...
  it('retries once with an error-correction prompt on malformed JSON, then succeeds', async () => {
    ...
    expect(result.intent.filters.property_type).toBe('studio');
    expect(result.intent.filters.max_price).toBeNull();
  ...
  it('throws a typed QueryUnderstandingError when the schema validates but a required field is missing', async () => {
    callClaudeMock.mockResolvedValueOnce(claudeResult(JSON.stringify({ filters: {} })));
    callClaudeMock.mockResolvedValueOnce(claudeResult(JSON.stringify({ filters: {} })));
