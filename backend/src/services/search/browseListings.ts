 ...
 * in one destination — no LLM, no embedding, no rerank. Price-ascending, deterministic
 * (`id` tiebreak). `destination` is a pre-validated slug from the route; the demo's
 * catalogue is ~35–37 rows per destination so there is no pagination.
 */
